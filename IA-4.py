...
main_windows =tk.Tk()
#set up the size
main_windows.geometry("312x324")
#Give it a name
main_windows.title('My_Calculator_GUI')
#Global variables
expression = ""
input_text = StringVar()
#create 3 functions
#function to hangle number/operation button click
def btn_click(item):
global expression
expression += str(item)
...
input_frame = tk.Frame(main_windows, width=312, height=50, bd=0, highlightbackground='black',
highlightcolor='black', highlightthickness=1)
input_frame.pack(side='top')
# Create an Entry widget (input field)
input_field =tk.Entry(input_frame, font=('arial', 18, 'bold'),
textvariable=input_text, width=50, bg="#fff", fg="black", bd=0, justify='right')
#postion the widget into the input_frame
input_field.grid(row=0, column=0)
input_field.pack(ipady=10)
# Buttons frame
btns_frame = tk.Frame(main_windows, width=312, height=272.5, bg="grey")
btns_frame.pack()
# The next row includes 2 buttons, 'Clear (Clear)' and 'Divide (/)':
...
#postion the button clear in the grid
...
#postion the button div in the grid
